...
void do_quantization(network net) {
    ...
    float input_quant_multiplier[TOTAL_CALIB_LAYER] = {0};
    float weight_quant_multiplier[TOTAL_CALIB_LAYER] = {0};

    FILE *fp = fopen("quant_multipliers.txt", "r");
    if (fp == NULL) {
        fprintf(stderr, "Error: Cannot open 'quant_multipliers.txt'. Please check the file path.\n");
        exit(1); 
    }

    for (int k = 0; k < TOTAL_CALIB_LAYER; ++k) {
        if (fscanf(fp, "%f %f", &input_quant_multiplier[k], &weight_quant_multiplier[k]) != 2) {
            fprintf(stderr, "Error: Invalid file format at line %d in 'quant_multipliers.txt'.\n", k + 1);
            fclose(fp);
            exit(1);
        }
    }
    fclose(fp);
    printf("Successfully loaded multipliers from quant_multipliers.txt\n\n");
