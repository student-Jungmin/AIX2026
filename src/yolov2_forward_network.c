...
void forward_convolutional_layer_cpu(layer l, network_state state)
{
    ...
    // static const char* tarDir = "/home/jung-min/Desktop/quantization";

    // char weightFile [100];  
    // char inputFile  [100];         
    // sprintf(weightFile, "%s/CONV%02d_weight.txt", tarDir, state.index);
    // sprintf(inputFile , "%s/CONV%02d_input.txt", tarDir, state.index); 
    
    // FILE *weightFp = fopen(weightFile, "w"); 
    // FILE *inputFp = fopen(inputFile, "a");

    // size_t weightNum = l.n * l.size * l.size * l.c;

    // for (int i = 0; i < weightNum; ++i) 
    //     fprintf(weightFp, "%0.7f ", l.weights[i]);

    // for (int i = 0; i < l.inputs; ++i)
    //     fprintf(inputFp, "%0.7f ", state.input[i]); 
